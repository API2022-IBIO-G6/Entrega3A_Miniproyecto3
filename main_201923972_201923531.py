...
for k in list_k:
    route_texton_dictionary = "data_mp3\\texton_dictionary\\texton_dictionary_{}.npy".format(
        k)  # Definimos la ruta para guardar el modelo de textons
...
for route in texton_dictionaries:
    ...
    features_texture_train = texture_201923972_201923531(images=train, labels=labels_train, route=route_train,
                                                         textons=texton_dictionary)
    ...
    features_texture_valid = texture_201923972_201923531(images=valid, labels=labels_valid, route=route_valid,
                                                         textons=texton_dictionary)
    predicted_labels = clasificador.predict(
        features_texture_valid)  # Obtenemos las predicciones(labels) para nuestras imagenes de valid
...
for k, predicted_labels in dic_labels_predicted_texture.items():
    ...
    df, precision, f1, recall = Resultados_Cuantitativos(true_labels=labels_valid, predicted_labels=predicted_labels,
...
for param1 in listParam1:
    for param2 in listParam2:
        ...
        features_train = shape_201923972_201923531(images=train, labels=labels_train, route=route_train, param1=param1,
                                                   param2=param2)
        clasificador = kmeans_classifier()  # Creamos el objeto de clase kmeans_classifier
        ...
        features_valid = shape_201923972_201923531(images=valid, labels=labels_valid, route=route_valid, param1=param1,
                                                   param2=param2)
...
true_labels = labels_valid  # Creamos el array con los labels
param1 = 32;
param2 = 32  # Mejor experiemnto shape
...
for feature in dic_features["train"]:
    for k in list_k:
        ...
        predicted_labels = clasificador.predict(
            features_valid)  # Obtenemos las predicciones(labels) para nuestras imagenes de valid
...
true_labels = labels_valid  # Creamos el array con los labels
feature = "color";
n_clusters = 20  # Mejor experiemnto
...
for key, index in dic.items():
    ...
    fig = Resultados_Cualitativos(index, images, true_labels, predicted_labels, columns=3)
    fig.suptitle(
        "Validación: {} del método Type:{}, color_space:{} con {} clusters".format(key, TYPE, colorSpace, n_clusters))
    fig.tight_layout()
    # fig.savefig("./data_mp3/ResultadosInforme/{}_shape_orientation{}_pixelCell{}.png".format(key,param1,param2))
    fig.show()
    continuar = input("\033[1;36m" + " Press Enter to continue..." + '\x1b[0m')
...
final_kmeans_model = kmeans_classifier(n_clusters).fit(dic_features["train"]["color"],
                                                       labels_train)  # Entrenamos el modelo
...
if os.path.exists(route_best_model):
    ...
    print("-->Classicador entrenado")
# --------------------------------TEST--------------------------
features_test = color_201923972_201923531(images=dic_images["test"], labels=dic_labels["test"], route=None, Type=TYPE,
                                          space_bins=spaceBins, color_space=colorSpace)
predicted_labels = clasificador.predict(
    features_test)  # Obtenemos las predicciones(labels) para nuestras imagenes de valid
print("-->Test terminado")
# ------------Resultados Cualitativos y Cuantitativos---------------
df, precision, f1, recall = Resultados_Cuantitativos(true_labels=dic_labels["test"], predicted_labels=predicted_labels,
                                                     unique_labels=unique_labels, name_experiment="TEST")
# Se halla el index de las imagenes bien clasificadas
index_clasificadas_bien = true_labels == predicted_labels
dic = {"Fortalezas": index_clasificadas_bien, "Debilidades": np.invert(index_clasificadas_bien)}
for key, index in dic.items():
    ...
    fig.suptitle(
        "Test: {} del método Type:{}, color_space:{} con {} clusters".format(key, TYPE, colorSpace, n_clusters))
    fig.tight_layout()
