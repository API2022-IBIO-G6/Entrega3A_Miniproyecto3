...
total_pixels = images_filter_response.shape[0] * images_filter_response.shape[1]
all_filter_response_train = np.array(np.split(images_filter_response.flatten(), total_pixels))
print(all_filter_response_train.shape)
...
def calculate_texton_histogram_201923972_201923531(filter_response, textons):
    k_bins = len(textons.keys())  # definimos el número de bins/clusters
    centers= np.array(list(textons.values()))
    model = KMeans(n_clusters=k_bins, random_state=0, init=centers, n_init=1).fit(centers)
    # hallamos los clusters asociado a cada filter_response (de un pixel)
    clusters = model.predict(filter_response)  
    hist, _ = np.histogram(clusters, bins=range(k_bins+1))  # calculamos el histograma de textones
    return hist, _
...
list_k = [4,8,12,16,20]  # definimos el número de clusters/textons de la experimentación
texton_dictionaries=[]
...
def Resultados_Cualitativos(index_clasificadas, images, true_labels, predicted_labels, columns, figsize=(10,10)):
    ...
    for i in range(len(images)): #Recorremos las imagenes
        img_gray =rgb2gray(plt.imread(images[i])) # al parecer también funciona image[:,:,0]
        ...
        if index_clasificadas[i]: #Si la imagen esta en la lista de index_clasificadas
            ...
            ax[c].imshow(hog_image, cmap=plt.cm.gray)
            ax[c].axis("Off")
            c+= 1
...
bins_textons = 4 #Mejor experimento Textura
...
bins_textons = 8 #Mejor experimento textura
best_texton_dictionary=np.load(os.path.join("data_mp3","texton_dictionary","texton_dictionary_{}.npy".format(bins_textons)), allow_pickle = True).item() 
param1 = 32; param2 = 32 #Mejor experiemnto shape
...
final_kmeans_model = kmeans_classifier(n_clusters).fit(dic_features["train"], labels_train)
